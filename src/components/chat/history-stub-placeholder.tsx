...
export function HistoryStubPlaceholder({
  ...
  return (
    <div
      ...
      className={cn(
        'group flex min-h-[48px] w-full max-w-[85%] items-center justify-center rounded-2xl bg-workspace-hover',
        'data-[failed=true]:cursor-pointer',
        role === 'user' ? 'ml-auto' : 'mr-auto',
        className
      )}
    >
      <span className="hidden text-xs text-workspace-muted group-data-[failed=true]:inline">
        Retry
      </span>
    </div>
  );
